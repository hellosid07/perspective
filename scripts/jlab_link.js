...
(async function() {
    try {
        ...
        if (!existsSync(link_path)) {
            throw Error("No @finos links found - run `yarn link` inside all Perspective packages first.");
        }
        ...
        console.log(app_path, staging_path);

        if (!existsSync(app_path) || !existsSync(staging_path)) {
